...
const STATUS_CORES = {
  pendente:     { bg: 'rgba(100,116,139,0.2)', color: '#94a3b8' },
  confirmado:   { bg: 'rgba(59,130,246,0.2)',  color: '#60a5fa' },
  em_producao:  { bg: 'rgba(245,158,11,0.2)',  color: '#fbbf24' },
  entregue:     { bg: 'rgba(16,185,129,0.2)',  color: '#34d399' },
...
const UNIDADES = [
  ...
  { key: 'PT', label: 'Pacote' }, { key: 'CX', label: 'Caixa' },
  { key: 'KG', label: 'Kg' }, { key: 'L', label: 'Litro' }, { key: 'M', label: 'Metro' },
]

function Badge({ status }) {
  const c = STATUS_CORES[status] || STATUS_CORES.pendente
...
const FORM_VAZIO = {
  cliente: '', orcamento: '',
  data_pedido: hoje(), entrega_prevista: '', status: 'pendente',
  observacoes: '', itens: [{ ...ITEM_VAZIO }],
}
            ...
            <div key={p.id}
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                  ...
                  {p.tipo === 'PRODUTO' && <span style={{ fontSize: 10, color: '#64748b' }}>· estoque: {parseFloat(p.quantidade_estoque).toLocaleString('pt-BR')}</span>}
                </div>
...
export default function PedidosPage() {
  ...
  useEffect(() => {
    ...
    api.get('/orcamentos/?status=aprovado&page_size=200').then(r => setOrcamentosAprovados(r.data.results || [])).catch(() => {})
  }, [])
  ...
  const abrirEditar = (o) => {
    ...
    setModal({
      ...
      data_pedido:      o.data_pedido || hoje(),
      entrega_prevista: o.entrega_prevista || '',
      status:           o.status,
  ...
  const excluir = async (id) => {
    setModalConfirmar({ msg: 'Excluir este pedido?', onConfirm: async () => { await api.delete('/pedidos/' + id + '/'); carregar(pagina) } })
  }
  ...
  const salvar = async () => {
    ...
    try {
      const payload = {
        ...
        orcamento:        modal.orcamento || null,
        data_pedido:      modal.data_pedido,
        entrega_prevista: modal.entrega_prevista || null,
        status:           modal.status,
        observacoes:      modal.observacoes,
        itens:            modal.itens.filter(i => i.descricao.trim()),
      }
      if (editandoId) await api.patch('/pedidos/' + editandoId + '/', payload)
  ...
  return (
    <SistemaLayout titulo="Pedidos">
      <div style={{ padding: '24px 28px', maxWidth: 1300, margin: '0 auto' }}>

        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: '#f1f5f9', margin: 0 }}>Pedidos</h1>
            <p style={{ fontSize: 13, color: '#a78bca', marginTop: 4 }}>{total} pedido{total !== 1 ? 's' : ''}</p>
          </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          {['', 'pendente', 'confirmado', 'em_producao', 'entregue', 'cancelado'].map(s => (
        ...
        <div style={card}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['Nº', 'Cliente', 'Entrega prevista', 'Itens', 'Total', 'Status', 'Ações'].map(h => (
            ...
            <tbody>
              {carregando ? (
                ...
                <tr><td colSpan={7} style={{ ...tdS, textAlign: 'center', color: '#a78bca', padding: 32 }}>Nenhum pedido encontrado</td></tr>
              ) : pedidos.map(o => (
                <tr key={o.id}
                  ...
                  <td style={tdS}>
                    {o.cliente_nome || '—'}
                    {o.orcamento_numero && <span style={{ fontSize: 10, color: '#a78bca', marginLeft: 5 }}>(orç. #{o.orcamento_numero})</span>}
                  </td>
                  <td style={tdS}>{o.entrega_prevista ? new Date(o.entrega_prevista + 'T00:00:00').toLocaleDateString('pt-BR') : '—'}</td>
                  <td style={{ ...tdS, textAlign: 'center' }}>{o.itens ? o.itens.length : 0}</td>
                  <td style={{ ...tdS, fontWeight: 600 }}>{fmt(o.subtotal)}</td>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 200, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '20px 16px', overflowY: 'auto' }}>
          <div style={{ background: '#0f0020', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, width: '100%', maxWidth: 860, padding: 28, marginTop: 20, marginBottom: 20 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 11, color: '#a78bca', marginBottom: 4, display: 'block' }}>Orçamento de origem (opcional)</label>
              <select value={modal.orcamento} onChange={e => setModal(m => ({ ...m, orcamento: e.target.value }))} style={inputStyle}>
                <option value="">Nenhum — pedido direto</option>
                {orcamentosAprovados.map(o => <option key={o.id} value={o.id}>#{o.numero} — {o.cliente_nome || o.prospecto_nome}</option>)}
              </select>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
              <div>
                <label style={{ fontSize: 11, color: '#a78bca', marginBottom: 4, display: 'block' }}>Data do pedido *</label>
                <input type="date" value={modal.data_pedido} onChange={e => setModal(m => ({ ...m, data_pedido: e.target.value }))} style={inputStyle} />
              </div>
              <div>
                <label style={{ fontSize: 11, color: '#a78bca', marginBottom: 4, display: 'block' }}>Entrega prevista</label>
                <input type="date" value={modal.entrega_prevista} onChange={e => setModal(m => ({ ...m, entrega_prevista: e.target.value }))} style={inputStyle} />
              </div>
              <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
              <div style={{ width: 280, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 15, fontWeight: 700 }}>
                  ...
                  <span style={{ color: '#34d399' }}>{fmt(sub)}</span>
                </div>
              </div>
            </div>

...
function badgeColor(segmento) {
  return segmentoCores[segmento?.toLowerCase()] || '#4b5563'
}
...
export default function ClientesPage() {
  ...
  const salvar = async (e) => {
    ...
    try {
      if (editando) {
        await api.patch(`/clientes/${editando}/`, form)
      } else {
    ...
    <div className="flex h-screen" style={{ backgroundColor: '#0d0020' }}>
            ...
            <div className="flex justify-center gap-2 mt-6">
              {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((pg) => (
                <button
                  ...
                  style={{
                    backgroundColor: pg === pagina ? '#063BF8' : 'rgba(255,255,255,0.06)',
      ...
      {modalAberto && (
        <div className="fixed inset-0 flex items-center justify-center z-50 px-4" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div
            className="w-full max-w-2xl rounded-2xl p-8 overflow-y-auto max-h-[90vh]"

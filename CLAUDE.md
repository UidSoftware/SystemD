...
| `livro-caixa/totais/` — saldo_atual | O card "Saldo Atual" deve ser `total_entradas - total_saidas` — **nunca** `ultimo.saldo_atual`. O `saldo_atual` armazenado em cada registro LivroCaixa é o saldo corrente da conta naquele momento (running balance), não o saldo líquido do período. Usar `ultimo.saldo_atual` retorna o valor da última transação, que diverge do total quando há saldo_inicial na conta ou qualquer inconsistência de corrida. |

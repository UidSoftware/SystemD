export default function EmailDetail({ email, onDeletar, onResponder }) {
  return (
        ...
        <div className="px-6 py-3" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <div className="flex flex-wrap gap-2">
            {email.anexos.map((a, i) => (
              <span key={i} className="text-xs px-3 py-1 rounded-full" style={{ backgroundColor: 'rgba(6,59,248,0.1)', color: '#6b8fff', border: '1px solid rgba(6,59,248,0.2)' }}>
                {a.nome}
              </span>
            ))}
          </div>
